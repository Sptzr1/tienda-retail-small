  ...
  export default async function Home() {
    ...
    if (session?.user) {
      ...
      // Si es admin, obtener todas las tiendas
      if (profile?.is_admin) {
        const { data: storesData } = await supabase.from("stores").select("*").order("name")
  
        stores = storesData || []
      } else if (profile?.store_id) {
        // Si no es admin, solo obtener su tienda asignada
        stores = profile.stores ? [profile.stores] : []
      }
    }
                ...
                <div className="mb-8">
                  ...
                  <p className="mt-1 text-sm text-gray-600">
                    {profile?.is_admin
                      ...
                      : profile?.store_id
                        ? `Estás asignado a: ${profile?.stores?.name}`
